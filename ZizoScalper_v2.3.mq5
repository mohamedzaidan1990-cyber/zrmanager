//+------------------------------------------------------------------+
//|                                              ZizoScalper_v2.3.mq5|
//|                                        Built for Mohamed @ Bayline|
//|           Hit & Run — v2.3 BREAKEVEN PROTECTION EDITION          |
//|  v1.8 base (PF=1.20) + BE move at 1x ATR — no trailing           |
//+------------------------------------------------------------------+
#property copyright "Zizo Scalper v2.3"
...
input double TP_Multiplier         = 2.0;   // back to v1.8 proven level
input double BreakevenATR          = 1.0;   // move SL to BE when profit >= this x ATR
...
//+------------------------------------------------------------------+
//| v2.3: Breakeven protection only — no trailing                    |
//| When profit >= BreakevenATR x ATR, move SL to open price.        |
//| Then let price run to full TP undisturbed.                        |
//+------------------------------------------------------------------+
void ManageBreakeven()
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(posInfo.PositionType() == POSITION_TYPE_BUY)
      {
         ...
         // Move to BE only once (when SL is still below open price)
         if(profit >= atr * BreakevenATR && currentSL < openPrice)
         {
      ...
      else if(posInfo.PositionType() == POSITION_TYPE_SELL)
      {
         ...
         // Move to BE only once (when SL is still above open price)
         if(profit >= atr * BreakevenATR && (currentSL > openPrice || currentSL == 0))
         {
            double newSL = openPrice - point;
            if(newSL > ask)
               trade.PositionModify(ticket, NormalizeDouble(newSL, _Digits), currentTP);
...
int OnInit()
{
   ...
   Print("=== ZIZO SCALPER v2.3 STARTED ===");
   Print("Breakeven protection at ", BreakevenATR, "x ATR | TP: ", TP_Multiplier, "x ATR | SL: ", SL_Multiplier, "x ATR");
   ...
   if(!isBacktesting)
      SendTelegram(VIPChatID, "*Zizo Scalper v2.3 ONLINE — Breakeven Edition*\n\n"
                            + "BE protection: ACTIVE at " + DoubleToString(BreakevenATR,1) + "x ATR\n"
                            + "No trailing — let winners run to full TP.\n"
...
void OnDeinit(const int reason)
{
   ...
   Print("=== ZIZO SCALPER v2.3 STOPPED ===");
   if(!isBacktesting)
      SendTelegram(VIPChatID, "*Zizo Scalper v2.3 OFFLINE*\n\nNo signals until restart.");
}
...
void OnTick()
{
   ...
   // Breakeven check runs on every tick
   ManageBreakeven();
   ...
   if(buySignal)
   {
      ...
      if(trade.Buy(lotSize, TradingSymbol, ask, sl, tp, "Zizo BUY v2.3 S"+IntegerToString(score)))
      {
   ...
   else if(sellSignal)
   {
      ...
      if(trade.Sell(lotSize, TradingSymbol, bid, sl, tp, "Zizo SELL v2.3 S"+IntegerToString(score)))
      {
