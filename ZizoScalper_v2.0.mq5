//+------------------------------------------------------------------+
//|                                              ZizoScalper_v2.0.mq5|
//|                                        Built for Mohamed @ Bayline|
//|               Hit & Run — v2.0 CANDLE BODY CONFIRMATION          |
//|  Crossover bar must close in signal direction — kills false breaks|
//+------------------------------------------------------------------+
#property copyright "Zizo Scalper v2.0"
...
// BUY: RSI 52-65 (momentum confirmed, not late)
// SELL: RSI 37-55 (restored — was 43% win rate at this zone)
...
//+------------------------------------------------------------------+
//| Scoring: trend EMA alignment determines lot size                  |
//| Score 2 = valid entry (1.0x risk)                                |
//| Score 3 = trend-aligned (1.3x risk)                              |
//+------------------------------------------------------------------+
...
int OnInit()
{
   ...
   Print("=== ZIZO SCALPER v2.0 STARTED ===");
   Print("Candle body filter: ON — crossover bar must close in signal direction");
   ...
   Print("Yalla yalla — Quality signals only!");
   ...
   if(!isBacktesting)
      SendTelegram(VIPChatID, "*Zizo Scalper v2.0 ONLINE*\n\n"
                            + "Candle body filter: ACTIVE\n"
                            + "BUY RSI: " + DoubleToString(RSI_BuyMin,0) + "-" + DoubleToString(RSI_BuyMax,0) + "\n"
                            + "SELL RSI: " + DoubleToString(RSI_SellMin,0) + "-" + DoubleToString(RSI_SellMax,0) + "\n"
...
void OnDeinit(const int reason)
{
   ...
   Print("=== ZIZO SCALPER v2.0 STOPPED ===");
   if(!isBacktesting)
      SendTelegram(VIPChatID, "*Zizo Scalper v2.0 OFFLINE*\n\nNo signals until restart.");
}
...
void OnTick()
{
   ...
   if(dailyLossCount >= MaxDailyLosses) { Print("Daily loss cap — stopped for today."); return; }
   ...
   // v2.0: Read candle open and close of the crossover bar (shift=1)
   double barClose = iClose(TradingSymbol, PERIOD_M5, 1);
   double barOpen  = iOpen(TradingSymbol,  PERIOD_M5, 1);
   bool bullishBar = (barClose > barOpen);   // candle closed up
   bool bearishBar = (barClose < barOpen);   // candle closed down
   ...
   // RSI zone check (asymmetric, no direction requirement — candle filter covers that)
   bool rsiBuyOk  = (rsi >= RSI_BuyMin  && rsi <= RSI_BuyMax);
   bool rsiSellOk = (rsi >= RSI_SellMin && rsi <= RSI_SellMax);

   // v2.0: Full entry conditions — crossover + RSI zone + candle body in signal direction
   bool buySignal  = emaBuyCross  && rsiBuyOk  && bullishBar && !buyCooldown;
   bool sellSignal = emaSellCross && rsiSellOk && bearishBar && !sellCooldown;
   ...
   if(buySignal)
   {
      ...
      Print("BUY S:", score, " Risk:", DoubleToString(riskPct,2), "% RSI:", DoubleToString(rsi,1),
            " Bar:BULLISH Trend:", ask>trendEMA?"WITH":"COUNTER");
      BroadcastSignal("BUY", ask, sl, tp, score);
      if(trade.Buy(lotSize, TradingSymbol, ask, sl, tp, "Zizo BUY v2.0 S"+IntegerToString(score)))
      {
   ...
   else if(sellSignal)
   {
      ...
      Print("SELL S:", score, " Risk:", DoubleToString(riskPct,2), "% RSI:", DoubleToString(rsi,1),
            " Bar:BEARISH Trend:", bid<trendEMA?"WITH":"COUNTER");
      BroadcastSignal("SELL", bid, sl, tp, score);
      if(trade.Sell(lotSize, TradingSymbol, bid, sl, tp, "Zizo SELL v2.0 S"+IntegerToString(score)))
      {
