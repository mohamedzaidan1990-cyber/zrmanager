//+------------------------------------------------------------------+
//|                                              ZizoScalper_v2.5.mq5|
//|                                        Built for Mohamed @ Bayline|
//|         Hit & Run — v2.5 TREND-ALIGNED BOTH DIRECTIONS           |
//|  BUY: price > 50 EMA only | SELL: price < 50 EMA only           |
//|  Both directions Score 3 — no counter-trend trades               |
//+------------------------------------------------------------------+
#property copyright "Zizo Scalper v2.5"
...
int OnInit()
{
   ...
   Print("=== ZIZO SCALPER v2.5 STARTED ===");
   Print("BUY: price > 50 EMA only | SELL: price < 50 EMA only");
   ...
   if(!isBacktesting)
      SendTelegram(VIPChatID, "*Zizo Scalper v2.5 ONLINE — Trend-Aligned Both Directions*\n\n"
                            + "BUY: price > 50 EMA only\n"
                            + "SELL: price < 50 EMA only\n"
...
void OnDeinit(const int reason)
{
   ...
   Print("=== ZIZO SCALPER v2.5 STOPPED ===");
   if(!isBacktesting)
      SendTelegram(VIPChatID, "*Zizo Scalper v2.5 OFFLINE*\n\nNo signals until restart.");
}
...
void OnTick()
{
   ...
   // v2.5: both directions require trend alignment (Score 3 only)
   bool trendAlignedBuy  = (ask > trendEMA);  // BUY only in uptrend
   bool trendAlignedSell = (bid < trendEMA);  // SELL only in downtrend

   // All accepted trades are Score 3 — always 1.30x risk
   double riskPct = BaseRiskPercent * 1.30;

   bool buySignal  = emaBuyCross  && rsiBuyOk  && trendAlignedBuy  && !buyCooldown;
   bool sellSignal = emaSellCross && rsiSellOk && trendAlignedSell && !sellCooldown;
   ...
   if(buySignal)
   {
      ...
      Print("BUY S3 Risk:", DoubleToString(riskPct,2), "% RSI:", DoubleToString(rsi,1), " Price>EMA50: YES");
      BroadcastSignal("BUY", ask, sl, tp);
      if(trade.Buy(lotSize, TradingSymbol, ask, sl, tp, "Zizo BUY v2.5"))
      {
   ...
   else if(sellSignal)
   {
      ...
      Print("SELL S3 Risk:", DoubleToString(riskPct,2), "% RSI:", DoubleToString(rsi,1), " Price<EMA50: YES");
      BroadcastSignal("SELL", bid, sl, tp);
      if(trade.Sell(lotSize, TradingSymbol, bid, sl, tp, "Zizo SELL v2.5"))
      {
