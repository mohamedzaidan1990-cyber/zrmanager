//+------------------------------------------------------------------+
//|                                              ZizoScalper_v2.7.mq5|
//|                                        Built for Mohamed @ Bayline|
//|         Hit & Run — v2.7 SESSION FILTER EDITION                  |
//|  v2.4 base (BUY filter > 50 EMA) + London + NY session only      |
//|  Avoids low-liquidity Asian/overnight noise on XAUUSD M5         |
//+------------------------------------------------------------------+
#property copyright "Zizo Scalper v2.7"
...
// Session filter — adjust to your broker's server timezone
// Raw Trading Ltd is typically UTC+2 (summer) / UTC+3 (winter DST)
// London open ~08:00 UTC = 10:00 server (UTC+2)
// NY close   ~21:00 UTC = 23:00 server (UTC+2)
// Default covers 09:00–22:00 server time (London open through NY close)
...
//+------------------------------------------------------------------+
//| Scoring — v2.4 logic: BUY Score 3 if above EMA, else Score 2    |
//+------------------------------------------------------------------+
...
bool IsOutsideSession()
{
   MqlDateTime dt;
   TimeToStruct(TimeCurrent(), dt);
   if(dt.hour < SessionStartHour || dt.hour >= SessionEndHour)
   {
      Print("Outside session (", dt.hour, "h) — skipping");
      return true;
   }
   return false;
}
...
int OnInit()
{
   ...
   Print("=== ZIZO SCALPER v2.7 STARTED ===");
   Print("Session filter: ", SessionStartHour, ":00 — ", SessionEndHour, ":00 server time");
   Print("BUY filter: price > 50 EMA | SELL: unfiltered (v2.4 base)");
   ...
   if(!isBacktesting)
      SendTelegram(VIPChatID, "*Zizo Scalper v2.7 ONLINE — Session Filter Edition*\n\n"
                            + "Active hours: " + IntegerToString(SessionStartHour) + ":00 – " + IntegerToString(SessionEndHour) + ":00 server time\n"
                            + "London + New York sessions only.\n"
                            + "BUY: price > 50 EMA | SELL: unfiltered\n"
...
void OnDeinit(const int reason)
{
   ...
   Print("=== ZIZO SCALPER v2.7 STOPPED ===");
   if(!isBacktesting)
      SendTelegram(VIPChatID, "*Zizo Scalper v2.7 OFFLINE*\n\nNo signals until restart.");
}
...
void OnTick()
{
   ...
   // v2.7 = v2.4 base: BUY requires price > 50 EMA, SELL unfiltered
   bool trendAlignedBuy = (ask > trendEMA);

   bool buySignal  = emaBuyCross  && rsiBuyOk  && trendAlignedBuy && !buyCooldown;
   bool sellSignal = emaSellCross && rsiSellOk && !sellCooldown;

   if(buySignal)
   {
      int    score   = ScoreSignal("BUY", ask, trendEMA); // always 3
      ...
      Print("BUY S3 session Risk:", DoubleToString(riskPct,2), "% RSI:", DoubleToString(rsi,1));
      BroadcastSignal("BUY", ask, sl, tp, score);
      if(trade.Buy(lotSize, TradingSymbol, ask, sl, tp, "Zizo BUY v2.7"))
      {
   ...
   else if(sellSignal)
   {
      ...
      Print("SELL S:", score, " session Risk:", DoubleToString(riskPct,2), "% RSI:", DoubleToString(rsi,1));
      BroadcastSignal("SELL", bid, sl, tp, score);
      if(trade.Sell(lotSize, TradingSymbol, bid, sl, tp, "Zizo SELL v2.7 S"+IntegerToString(score)))
      {
