//+------------------------------------------------------------------+
//|                                              ZizoScalper_v2.1.mq5|
//|                                        Built for Mohamed @ Bayline|
//|               Hit & Run — v2.1 H1 REGIME FILTER EDITION          |
//|  Only scalp against H1 trend direction (catch exhaustion moves)  |
//+------------------------------------------------------------------+
#property copyright "Zizo Scalper v2.1"
...
// H1 regime filter period — 50 EMA on H1
// When H1 price > H1_50EMA: uptrend — only take SELL scalps (fade the tops)
// When H1 price < H1_50EMA: downtrend — only take BUY scalps (fade the bottoms)
input int    H1_RegimePeriod       = 50;
...
int handleTrendEMA = INVALID_HANDLE;  // M5 50 EMA for lot scoring
int handleH1EMA    = INVALID_HANDLE;  // H1 50 EMA for regime filter
...
void BroadcastSignal(string direction, double entry, double sl, double tp, int score, string regime)
{
...
//+------------------------------------------------------------------+
//| Scoring: M5 trend EMA for lot size only                          |
//+------------------------------------------------------------------+
int ScoreSignal(string direction, double price, double m5TrendEMA)
{
   bool trendBuy  = (direction == "BUY"  && price > m5TrendEMA);
   bool trendSell = (direction == "SELL" && price < m5TrendEMA);
...
int OnInit()
{
   ...
   handleH1EMA    = iMA(TradingSymbol, PERIOD_H1, H1_RegimePeriod,0, MODE_EMA, PRICE_CLOSE);
   ...
   if(handleFastEMA==INVALID_HANDLE || handleSlowEMA==INVALID_HANDLE ||
      handleTrendEMA==INVALID_HANDLE || handleH1EMA==INVALID_HANDLE  ||
   ...
   Print("=== ZIZO SCALPER v2.1 STARTED ===");
   Print("H1 Regime filter: H1 50 EMA | Uptrend=SELL only | Downtrend=BUY only");
   Print("BUY RSI: ", RSI_BuyMin, "-", RSI_BuyMax, " | SELL RSI: ", RSI_SellMin, "-", RSI_SellMax);
   Print("Daily loss cap: ", MaxDailyLosses, " | Mode: ", isBacktesting ? "BACKTEST" : "LIVE");
   Print("Yalla yalla — Regime-aware scalping activated!");
   ...
   if(!isBacktesting)
      SendTelegram(VIPChatID, "*Zizo Scalper v2.1 ONLINE*\n\n"
                            + "H1 Regime filter: ACTIVE\n"
                            + "Uptrend: SELL scalps only\n"
                            + "Downtrend: BUY scalps only\n"
...
void OnDeinit(const int reason)
{
   ...
   if(handleH1EMA    != INVALID_HANDLE) { IndicatorRelease(handleH1EMA);    handleH1EMA    = INVALID_HANDLE; }
   ...
   Print("=== ZIZO SCALPER v2.1 STOPPED ===");
   if(!isBacktesting)
      SendTelegram(VIPChatID, "*Zizo Scalper v2.1 OFFLINE*\n\nNo signals until restart.");
}
...
void OnTick()
{
   ...
   double m5TrendEMA   = GetValue(handleTrendEMA, 1);
   double rsi          = GetValue(handleRSI,      1);
   double atr          = GetValue(handleATR,      1);
   double atr50        = GetValue(handleATR50,    1);

   // H1 regime — last closed H1 bar
   double h1EMA        = GetValue(handleH1EMA, 1);
   double h1Close      = iClose(TradingSymbol, PERIOD_H1, 1);

   if(fastEMA_cur==0 || slowEMA_cur==0 || atr==0 || h1EMA==0) return;
   if(IsRangingMarket(atr)) return;

   // v2.1: H1 regime filter
   // Uptrend   (h1Close > h1EMA): fade the micro-tops — SELL only
   // Downtrend (h1Close < h1EMA): fade the micro-bottoms — BUY only
   bool h1Uptrend   = (h1Close > h1EMA);
   bool h1Downtrend = (h1Close < h1EMA);
   string regime    = h1Uptrend ? "UPTREND" : "DOWNTREND";
   ...
   // Full signals — H1 regime blocks the wrong direction
   bool buySignal  = emaBuyCross  && rsiBuyOk  && h1Downtrend && !buyCooldown;
   bool sellSignal = emaSellCross && rsiSellOk && h1Uptrend   && !sellCooldown;

   if(buySignal)
   {
      int    score   = ScoreSignal("BUY", ask, m5TrendEMA);
      ...
      Print("BUY S:", score, " Risk:", DoubleToString(riskPct,2), "% RSI:", DoubleToString(rsi,1),
            " H1:", regime, " (fading bottom)");
      BroadcastSignal("BUY", ask, sl, tp, score, regime);
      if(trade.Buy(lotSize, TradingSymbol, ask, sl, tp, "Zizo BUY v2.1 S"+IntegerToString(score)))
      {
   ...
   {
      int    score   = ScoreSignal("SELL", bid, m5TrendEMA);
      double riskPct = BaseRiskPercent * RiskMultiplier(score);
      double sl_dist = atr * sl_multi;
      double lotSize = CalculateLotSize(sl_dist, riskPct);
      if(lotSize <= 0) return;
      ...
      Print("SELL S:", score, " Risk:", DoubleToString(riskPct,2), "% RSI:", DoubleToString(rsi,1),
            " H1:", regime, " (fading top)");
      BroadcastSignal("SELL", bid, sl, tp, score, regime);
      if(trade.Sell(lotSize, TradingSymbol, bid, sl, tp, "Zizo SELL v2.1 S"+IntegerToString(score)))
      {
