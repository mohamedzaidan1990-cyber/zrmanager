//+------------------------------------------------------------------+
//|                                              ZizoScalper_v2.4.mq5|
//|                                        Built for Mohamed @ Bayline|
//|         Hit & Run — v2.4 LONG QUALITY FILTER EDITION             |
//|  v1.8 base (no exit mgmt) + BUY requires price > 50 EMA          |
//|  Shorts: 43% win rate. Longs drag it down. Fix the long filter.  |
//+------------------------------------------------------------------+
#property copyright "Zizo Scalper v2.4"
...
int OnInit()
{
   ...
   Print("=== ZIZO SCALPER v2.4 STARTED ===");
   Print("BUY filter: price MUST be above 50 EMA | SELL: score 2 or 3 accepted");
   Print("TP: ", TP_Multiplier, "x ATR | SL: ", SL_Multiplier, "x ATR | No exit management");
   ...
   if(!isBacktesting)
      SendTelegram(VIPChatID, "*Zizo Scalper v2.4 ONLINE — Long Quality Filter*\n\n"
                            + "BUY: only when price > 50 EMA\n"
                            + "SELL: unchanged\n"
...
void OnDeinit(const int reason)
{
   ...
   Print("=== ZIZO SCALPER v2.4 STOPPED ===");
   if(!isBacktesting)
      SendTelegram(VIPChatID, "*Zizo Scalper v2.4 OFFLINE*\n\nNo signals until restart.");
}
...
void OnTick()
{
   ...
   // KEY v2.4 CHANGE: BUY only when price is above the 50 EMA (Score 3 only)
   ...
   if(buySignal)
   {
      int    score   = ScoreSignal("BUY", ask, trendEMA); // will always be 3 here
      ...
      if(trade.Buy(lotSize, TradingSymbol, ask, sl, tp, "Zizo BUY v2.4 S3"))
      {
   ...
   else if(sellSignal)
   {
      ...
      if(trade.Sell(lotSize, TradingSymbol, bid, sl, tp, "Zizo SELL v2.4 S"+IntegerToString(score)))
      {
