//+------------------------------------------------------------------+
//|                                              ZizoScalper_v2.2.mq5|
//|                                        Built for Mohamed @ Bayline|
//|               Hit & Run — v2.2 TRAILING STOP EDITION             |
//|  BE at 1x ATR, trail at 1.5x ATR — let winners run further       |
//+------------------------------------------------------------------+
#property copyright "Zizo Scalper v2.2"
...
input double TP_Multiplier         = 2.5;   // raised to give trail room
// Trailing stop levels (multiples of ATR)
input double TrailBreakevenATR     = 1.0;   // move SL to BE when profit >= this x ATR
input double TrailStartATR         = 1.5;   // start trailing when profit >= this x ATR
input double TrailDistanceATR      = 0.5;   // trail SL this many x ATR behind current price
...
//+------------------------------------------------------------------+
//| Scoring: M5 50 EMA trend alignment for lot sizing                |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| v2.2: Trailing stop — runs on every tick                         |
//+------------------------------------------------------------------+
void ManageTrailingStop()
{
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(posInfo.Symbol() != TradingSymbol) continue;
      if(posInfo.Magic() != 20260510) continue;
      ...
      if(posInfo.PositionType() == POSITION_TYPE_BUY)
      {
         ...
         if(profit >= atr * TrailBreakevenATR && currentSL < openPrice)
            newSL = openPrice + point;

         // Step 2: trail when further in profit
         if(profit >= atr * TrailStartATR)
         {
            double trailLevel = currentBid - atr * TrailDistanceATR;
      ...
      else if(posInfo.PositionType() == POSITION_TYPE_SELL)
      {
         ...
         // Step 1: move to breakeven
         if(profit >= atr * TrailBreakevenATR && (currentSL > openPrice || currentSL == 0))
            newSL = openPrice - point;

         // Step 2: trail when further in profit
         if(profit >= atr * TrailStartATR)
         {
            double trailLevel = currentAsk + atr * TrailDistanceATR;
            if(trailLevel < newSL || newSL == 0) newSL = trailLevel;
         }

         if((newSL < currentSL || currentSL == 0) && newSL > currentAsk)
            trade.PositionModify(ticket, NormalizeDouble(newSL, _Digits), currentTP);
      }
   }
}
...
int OnInit()
{
   ...
   Print("=== ZIZO SCALPER v2.2 STARTED ===");
   Print("Trailing stop: BE at ", TrailBreakevenATR, "x ATR | Trail at ", TrailStartATR, "x ATR | Distance ", TrailDistanceATR, "x ATR");
   ...
   if(!isBacktesting)
      SendTelegram(VIPChatID, "*Zizo Scalper v2.2 ONLINE — Trailing Stop Edition*\n\n"
                            + "Trailing stop: ACTIVE\n"
                            + "BE at: " + DoubleToString(TrailBreakevenATR,1) + "x ATR\n"
                            + "Trail at: " + DoubleToString(TrailStartATR,1) + "x ATR\n"
...
void OnDeinit(const int reason)
{
   ...
   Print("=== ZIZO SCALPER v2.2 STOPPED ===");
   if(!isBacktesting)
      SendTelegram(VIPChatID, "*Zizo Scalper v2.2 OFFLINE*\n\nNo signals until restart.");
}
...
void OnTick()
{
   ...
   // Trailing stop runs on EVERY tick (not locked to bar)
   ManageTrailingStop();

   // Entry logic locked to new closed bar only
   ...
   if(buySignal)
   {
      ...
      if(trade.Buy(lotSize, TradingSymbol, ask, sl, tp, "Zizo BUY v2.2 S"+IntegerToString(score)))
      {
   ...
   else if(sellSignal)
   {
      ...
      if(trade.Sell(lotSize, TradingSymbol, bid, sl, tp, "Zizo SELL v2.2 S"+IntegerToString(score)))
      {
